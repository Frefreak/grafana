...
var (
	...
	logger = logging.DefaultLogger.With("logger", "lease-manager")
)
...
func NewManager(store kv.KV, holder string, opts ...ManagerOption) *Manager {
	...
	m.garbageCollector = newGarbageCollector(store, m.now)
...
func (m *Manager) RunGarbageCollection(ctx context.Context) (int, error) {
	return newGarbageCollector(m.store, m.now).runOnce(ctx)
}
...
func (m *Manager) autoRenewLoop(lease *Lease, expiry time.Time, ttl, renewInterval time.Duration) {
	...
	for {
		log := logger.With("lease", lease.key.name, "holder", lease.holder, "generation", lease.key.generation)
